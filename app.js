...
$('#printButton').addEventListener('click',()=>{const jobs=state.jobs.filter(j=>j.status!=='reading');if(!jobs.length)return;$('#printArea').innerHTML=jobs.map(j=>mapPage(j)+labelPage(j)).join('');setTimeout(()=>window.print(),250);});
